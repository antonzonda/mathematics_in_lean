...
example (lbf : FnHasLb f) (lbg : FnHasLb g) : FnHasLb fun x ↦ f x + g x := by
  ...
  have h := fun x ↦ add_le_add (lbfa x) (lbgb x)
  use a + b, h

example {c : ℝ} (ubf : FnHasUb f) (h : c ≥ 0) : FnHasUb fun x ↦ c * f x := by
  rcases ubf with ⟨a, upfa⟩
  have h := fun x ↦ mul_le_mul_of_nonneg_left (upfa x) h
  use c * a, h
...
example (surjg : Surjective g) (surjf : Surjective f) : Surjective fun x ↦ g (f x) := by
  intro z
  rcases surjg z with ⟨a, ga⟩
  rcases surjf a with ⟨b, fb⟩
  use b
  dsimp
  rw [fb, ga]
